...
export function CalendarView({ view = "week" }: CalendarViewProps) {
  ...
  const isBooked = (labId: string, date: string, time: string) => {
    return bookings.some(
      (booking: any) =>
        booking.labId === labId && booking.date === date && booking.startTime <= time && booking.endTime > time,
    )
  }
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64">Loading calendar data...</div>
  }

  // Update the days array to include weekends:
  const days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]

  // Update the dayMap to include Saturday and Sunday:
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2">
          <Button variant="outline" size="icon" onClick={currentView === "week" ? prevWeek : prevDay}>
          ...
          <Button variant="outline" size="icon" onClick={currentView === "week" ? nextWeek : nextDay}>
            ...
            <div className="space-y-2 pt-2">
              {labs.map((lab: any) => (
                <div key={lab._id} className="grid grid-cols-8 gap-2">
                  <div className="flex items-center font-medium">{lab.name}</div>
                  {weekDays.map((day, i) => {
                    const dateStr = format(day, "yyyy-MM-dd")
                    const hasBooking = bookings.some((b: any) => b.labId === lab._id && b.date === dateStr)
                    const dayBookings = bookings.filter((b: any) => b.labId === lab._id && b.date === dateStr)

                    return (
                      <div
                        key={i}
                        className={`flex h-16 items-center justify-center rounded-md border p-2 ${
                          hasBooking ? "bg-primary/10 border-primary/20" : "bg-muted/40"
                        }`}
                      >
                        {hasBooking ? (
                          <TooltipProvider>
                              ...
                              <TooltipTrigger asChild>
                                <div className="flex flex-col items-center">
                                  {dayBookings.length > 1 ? (
                                    ...
                                    <Badge
                                      variant="outline"
                                      className={`${dayBookings[0].isExam ? "bg-red-100 text-red-800 border-red-300" : getYearGroupColor(dayBookings[0].yearGroup)}`}
                                    >
                                      {dayBookings[0].isExam ? "Exam" : `Year ${dayBookings[0].yearGroup}`}
                                    </Badge>
                                  )}
                              ...
                              <TooltipContent>
                                <div className="space-y-1 text-xs">
                                  {dayBookings.map((booking: any) => (
                                    <div key={booking._id} className="border-b pb-1 last:border-0 last:pb-0">
                                      ...
                                      <div className="flex items-center gap-1 mt-1">
                                        <Badge
                                          variant="outline"
                                          className={`${booking.isExam ? "bg-red-100 text-red-800 border-red-300" : getYearGroupColor(booking.yearGroup)}`}
                                        >
                                          {booking.isExam ? "Exam" : `Year ${booking.yearGroup}`}
                                        </Badge>
                                      </div>
                                    </div>
                                  ))}
          ...
          <div className="min-w-[800px]">
            ...
            <div className="space-y-2 pt-2">
              {timeSlots.map((time, i) => {
                ...
                return (
                  <div key={i} className="grid grid-cols-[100px_repeat(5,1fr)] gap-2">
                      ...
                      return (
                        <div
                          ...
                          className={`flex h-12 items-center justify-center rounded-md border p-2 ${
                            booked ? "bg-primary/10 border-primary/20" : "bg-muted/40"
                          }`}
                        >
                          {booked ? (
                            <TooltipProvider>
                                ...
                                <TooltipTrigger asChild>
                                  <div className="flex items-center gap-2">
                                    ...
                                    <Badge
                                      ...
                                      size="sm"
                                    >
                                      {booking?.isExam ? "Exam" : `Y${booking?.yearGroup}`}
                                    </Badge>
                                  </div>
                                </TooltipTrigger>
                                <TooltipContent>
