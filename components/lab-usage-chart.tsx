...
import { Bar, BarChart, ResponsiveContainer, XAxis, YAxis, Tooltip, Legend, Cell } from "@/components/ui/chart"
...
export function LabUsageChart() {
  ...
  const processUsageData = (labs: any[], bookings: any[], selectedLabId: string) => {
    // Filter bookings by selected lab if needed
    const filteredBookings =
      selectedLabId === "all" ? bookings : bookings.filter((booking: any) => booking.labId === selectedLabId)
    ...
    const labUsage = labs.map((lab: any) => {
      const labBookings = bookings.filter((booking: any) => booking.labId === lab._id)
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64">Loading lab usage data...</div>
  }
          ...
          <CardContent>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={selectedLab === "all" ? usageData : usageData.filter((lab) => lab.id === selectedLab)}
                  layout="vertical"
                ...
                >
                  ...
                  <Legend />
                  <Bar dataKey="bookedHours" stackId="a" fill="#8884d8" name="Booked Hours" />
                  <Bar dataKey="freeHours" stackId="a" fill="#82ca9d" name="Free Hours" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
