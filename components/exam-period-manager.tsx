...
} from "@/app/actions/exam-period-actions";
...
export function ExamPeriodManager() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const [examPeriodsRes, labsRes] = await Promise.all([
          fetch("/api/exam-periods"),
          fetch("/api/labs"),
        ]);

        if (examPeriodsRes.ok && labsRes.ok) {
          const [examPeriodsData, labsData] = await Promise.all([
            examPeriodsRes.json(),
            labsRes.json(),
          ]);
  ...
  const handleToggleExamPeriod = async (id: string, isActive: boolean) => {
    try {
      ...
      if (result.success) {
        ...
        setExamPeriods(
          examPeriods.map((period: any) =>
            period._id === id ? { ...period, isActive: !isActive } : period
          )
        );
      } else {
        toast({
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="grid gap-2">
            ...
            <Select>
              <SelectTrigger>
                <SelectValue
                  placeholder={`${selectedLabs.length} labs selected`}
                />
              </SelectTrigger>
              <SelectContent>
                {labs.map((lab: any) => (
                  <SelectItem
                    ...
                    className={
                      selectedLabs.includes(lab._id) ? "bg-primary/10" : ""
                    }
                  >
                    {lab.name}
                  </SelectItem>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        <Badge
                          variant="outline"
                          className={getYearGroupColor(period.yearGroup)}
                        >
                    ...
                    <div className="flex space-x-2">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          handleToggleExamPeriod(period._id, period.isActive)
                        }
                        disabled={
                          isToggling === period._id || isDeleting !== null
                        }
                      >
                        {isToggling === period._id
                          ? "Updating..."
                          : period.isActive
                          ? "Deactivate"
                          : "Activate"}
                      ...
                      <Button
                        ...
                        disabled={
                          isDeleting === period._id || isToggling !== null
                        }
                      >
