...
export function BookingRequests() {
  ...
  const handleApprove = async (id: number) => {
    try {
      ...
      const result = await updateBookingStatus(id.toString(), "approved");
  ...
  const handleReject = async (id: number) => {
    try {
      ...
      const result = await updateBookingStatus(id.toString(), "rejected");
        ...
        <div key={request._id} className="rounded-lg border p-4">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
            <div className="flex items-center text-muted-foreground">
              ...
              {/* {format(parseISO(request.date), "MMMM dd yyyy")} */}
            </div>
            <div className="flex items-center text-muted-foreground">
