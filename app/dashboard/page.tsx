...
async function getDashboardData() {
  try {
    ...
    const labsRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/api/labs`, {
      next: { revalidate: 60 },
      cache: "no-store",
    })
    const bookingsRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/api/bookings`, {
      next: { revalidate: 60 },
      cache: "no-store",
    })
    const examPeriodsRes = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/api/exam-periods`,
      {
        next: { revalidate: 60 },
        cache: "no-store",
      },
    ...
    const availableNow =
      ...
      (bookings.filter((b) => {
        ...
        return b.date === today && b.startTime <= currentHour && b.endTime > currentHour
      }).length || 0)
...
export default function DashboardPage() {
  return (
    <DashboardShell>
      <DashboardHeader heading="Dashboard" text="Manage your lab bookings and view availability">
        ...
        <TabsContent value="calendar" className="space-y-4">
          <Card>
            <CardHeader className="space-y-1">
              ...
              <CardDescription>View and manage lab availability</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <Suspense fallback={<Skeleton className="h-[400px] w-full" />}>
                <UpcomingBookings userId={2} /> {/* In a real app, this would be the current user's ID */}
              </Suspense>
            </CardContent>
          </Card>
        </TabsContent>
        <TabsContent value="reports" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Analytics and insights on lab usage</CardDescription>
            </CardHeader>
            <CardContent>
              <Suspense fallback={<Skeleton className="h-[400px] w-full" />}>
                <BookingStats />
...
async function DashboardContent() {
  ...
  const userBookings = data.bookings.filter((b) => b.userId === 2)
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{data.totalLabs - data.availableNow} labs currently in use</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{upcomingUserBookings} upcoming</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {data.upcomingExamPeriod
                ? `${new Date(data.upcomingExamPeriod.startDate).toLocaleDateString()} - ${new Date(data.upcomingExamPeriod.endDate).toLocaleDateString()}`
                : "None scheduled"}
            </div>
            <p className="text-xs text-muted-foreground">
              {data.upcomingExamPeriod ? data.upcomingExamPeriod.name : "No upcoming exam periods"}
            </p>
          </CardContent>
        </Card>
      </div>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="col-span-4">
          <CardHeader>
            ...
            <CardDescription>Lab utilization for the current week</CardDescription>
          </CardHeader>
        ...
        <Card className="col-span-3">
          ...
          <CardContent>
            <UpcomingBookings userId={2} limit={3} /> {/* In a real app, this would be the current user's ID */}
          </CardContent>
        </Card>
      </div>
    </>
